...
def get_issue_data(issue_url):
    ...
    if not match:
        st.error("Invalid GitHub issue URL format.")
        return None, None
    ...
    if issue_resp.status_code != 200:
        st.error("Could not fetch the issue from GitHub.")
        return None, None
...
def extract_status_by_finance(issue, comments):
    """
    Only treat status as Approved/Rejected if the author of the issue/comment is a finance user.
    Otherwise, always return Under Review.
    """
    # Check issue body for explicit status set by author
    author_login = issue.get('user', {}).get('login', '').lower()
    body = issue.get('body', '').lower() if issue.get('body') else ""
    status = None

    # Only allow Approved/Rejected if set by finance author
